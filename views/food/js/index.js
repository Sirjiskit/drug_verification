...
function btnAction($table) {
    $('#tb-food tbody, .dtr-details').on('click', '.btn-action', async function (e) {
        e.preventDefault();
        var fd = new FormData();
        var id = parseInt($(this).attr("data-id"));
        var status = parseInt($(this).attr("data-action"));
        var old = parseInt($(this).attr('data-old'));
        var url = window.siteurl + `food/update`;
        fd.append('id', id);
        fd.append('status', status);
        var conf = await confirm({message: `Are you sure you want to ${['', 'approve', 'reject'][status]} this product?`});
